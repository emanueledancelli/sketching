...
module.exports = (args) => {
  ...
  console.log(
    chalk.blue(`
  Sketching - Emanuele Dancelli - https://github.com/emanueledancelli/sketching
    `)
  );
  ...
  os.execCommand(`mkdir ${title}`)
    .then(() => {
      ...
      os.execCommand(`cp -R ${path}/* ${title}`).then(() => {
        spinner.text = `Creating your new sketchboard ~ ${title} ~ Downloading and installing dependencies (parcel-budler and p5). This might take a minute.`;
